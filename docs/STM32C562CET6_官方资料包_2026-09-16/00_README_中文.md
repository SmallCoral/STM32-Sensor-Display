...
资料包对应 STM32C562CET6：STM32C5 系列、Cortex-M33、144 MHz、512 KB Flash、128 KB SRAM、LQFP48、温度等级 6（环境温度 -40 至 +85 °C，结温最高 +105 °C）。
...
3. `official/07_MB2213_NUCLEO-C562RE_Official_Schematic.pdf`
   ...
   - 该板使用 C562RE/LQFP64，参考电路拓扑时需要重新映射引脚。
...
MCU 附近除 3 个 100 nF 外，再放至少 4.7 µF 的总去耦。所有 VDD/VSS 均需连接。电源范围为 2.7～3.6 V，USB VBUS 的 5 V 经过 3.3 V 稳压器后再接 MCU VDD。
...
- C562 内置 USB D+/D- 匹配阻抗和 D+ 上拉，通常不需要外置 22 Ω 串联电阻或 1.5 kΩ D+ 上拉。
...
- USB DFU 用于烧录和擦除，不能替代 SWD 的断点、单步和寄存器调试。需要在线调试时应引出 3V3、GND、SWDIO、SWCLK 和 NRST。

## PCB 核对项

- USB D+/D- 保持成对、短而连续，尽量少换层和打孔，并远离开关电源与高 dv/dt 节点；ESD 器件靠近接口。
- VCAP 2.2 µF、每个 VDD 的 100 nF 都应紧靠对应引脚，回地路径要短。
- HSE、LSE 是否安装由具体应用决定；USB DFU 本身不依赖外部晶振。
- FDCAN 仍需外部收发器，MCU 的 TX/RX 不能直接连接 CANH/CANL。120 Ω 终端只在总线端点启用。
- LQFP48 只有 38 个可用 I/O，USB、SWD、FDCAN 和晶振会占用复用资源；确定接口前先做一次完整的 Pinout 冲突检查。
- 生产前重新核对 DS14927、RM0522、AN2606、AN6274 和 ES0661 的最新版本，重点检查勘误表。
